...
> **Примечание**: Скриншоты конфигурации кластера и сервисов доступны в [docs/Screen.md](docs/Screen.md).
...
| Infra | Инфраструктурные сервисы | 2 | 4 | 8 GB | node-role=infra |
...
## 🔄 CI/CD Pipeline (GIGACODE)

В качестве CI/CD pipeline используется агентная система GIGACODE.

### Обзор GIGACODE

GIGACODE — это агентная система автоматизации DevOps процессов, которая обеспечивает:

- **Автоматизацию развертывания**: Выполнение скриптов Terraform и Helm через агенты
- **Управление конфигурациями**: Централизованное управление параметрами окружения
- **Мониторинг выполнения**: Реальное время выполнения pipeline'ов
- **Безопасность**: Изолированное выполнение задач с контрольными точками

### Этапы pipeline

1. **Checkout кода**: Получение последней версии из репозитория
2. **Валидация конфигурации**: Проверка Terraform и Kubernetes манифестов
3. **Развертывание инфраструктуры**: Выполнение Terraform apply
4. **Настройка кластера**: Создание и настройка Kubernetes
5. **Развертывание приложения**: Установка через Helm
6. **Проверка работоспособности**: Запуск тестов и валидация
7. **Сообщение о результате**: Уведомление о статусе pipeline
...
### Доступ к приложению
После развертывания приложение Online Boutique будет доступно из браузера через внешний IP-адрес LoadBalancer сервиса `frontend-external`.
Получите внешний IP-адрес с помощью команды:
```bash\nkubectl get service frontend-external -n online-boutique
```
Вывод будет содержать внешний IP-адрес в столбце `EXTERNAL-IP`. Используйте этот IP-адрес для доступа к приложению в браузере:
🌐 **http://<EXTERNAL-IP>/
...
*Этот проект создан в рамках кур��а Otus AI First Kubernetes.*
